    ...
    for (int j = 0; j < NUMBERS.size(); ++j) {
        ...
        if (NUMBERS[j]->checkDigitIncreaseConsistency) {
            if (flowDigit) {
                ...
                else if (PreValueUse && NUMBERS[j]->PreValueOkay) {
        ...
        if (!NUMBERS[j]->AllowNegativeRates && (NUMBERS[j]->Value < NUMBERS[j]->PreValue) && PreValueUse && NUMBERS[j]->PreValueOkay) {
            ...
            if ((NUMBERS[j]->Value >= (NUMBERS[j]->PreValue-(2/pow(10, NUMBERS[j]->Nachkomma))) && NUMBERS[j]->isExtendedResolution)
                ...
                NUMBERS[j]->ErrorMessageText = NUMBERS[j]->ErrorMessageText + "Neg. Rate - Processed Value: " + RundeOutput(NUMBERS[j]->Value, NUMBERS[j]->Nachkomma) + " - Raw: " + NUMBERS[j]->ReturnRawValue + " - Pre: " + RundeOutput(NUMBERS[j]->PreValue, NUMBERS[j]->Nachkomma) + " "; 
                NUMBERS[j]->Value = NUMBERS[j]->PreValue;
                NUMBERS[j]->ReturnValue = "";
                NUMBERS[j]->lastvalue = imagetime;
        ...
        if (NUMBERS[j]->useMaxRateValue && (NUMBERS[j]->Value != NUMBERS[j]->PreValue) && PreValueUse && NUMBERS[j]->PreValueOkay) {
            ...
            if (abs(_ratedifference) > abs(NUMBERS[j]->MaxRateValue)) {
                NUMBERS[j]->ErrorMessageText = NUMBERS[j]->ErrorMessageText + "Rate too high - Processed Value: " + RundeOutput(NUMBERS[j]->Value, NUMBERS[j]->Nachkomma) + " - Pre: " + RundeOutput(NUMBERS[j]->PreValue, NUMBERS[j]->Nachkomma) + " - Rate: " + RundeOutput(_ratedifference, NUMBERS[j]->Nachkomma);
                NUMBERS[j]->Value = NUMBERS[j]->PreValue;
                NUMBERS[j]->ReturnValue = "";
                NUMBERS[j]->ReturnRateValue = "";
                NUMBERS[j]->lastvalue = imagetime;
